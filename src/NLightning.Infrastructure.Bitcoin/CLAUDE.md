...
- `SecureKeyManager` writes v2 key files (random 16-byte salt, random 24-byte nonce, Argon2id 64 MiB/3 passes, all stored in the file). v1 files (fixed `s_legacySalt`, zero nonce, 64 KiB) are still read and are rewritten as v2 by `FromFilePath` (atomic temp-file + move). The node key (`GetNodeKeyPair`) is the master key itself.
