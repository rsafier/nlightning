...
- Every packet-caused `Peel` failure is an `OnionException` (only caller bugs, such as a bad node key or a `default` packet, throw anything else): BADONION codes (`InvalidOnionVersion`/`Key`/`Hmac`/`Blinding`) carry sha256_of_onion as `FailureData`; framing errors are `InvalidOnionPayload` with data `00 0000` (type 0, offset 0). Semantic hop-payload validation is not done here.
