...
- `BlockchainMonitorService` block failures (NL-097): a block whose `ProcessBlock` throws is retried `MaxBlockProcessingAttempts` (3) times with doubling backoff from `BlockRetryBaseDelay` (1s); then the round halts with a Critical log, leaving it and later blocks queued (never skipped). The next ZMQ block or a restart retries from it. Retries re-raise `OnNewBlockDetected`, and `ProcessNewBlock` still saves the uow after a halted round, so partial effects of the failed attempt persist.
