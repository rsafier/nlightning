...
/// on its record after accepting it as our final hop (B5-LCL-RO-02), and the switch is asked to decide while it has not.
/// </summary>
/// <remarks>
...
/// settle. An HTLC whose onion made us an intermediate hop never gets it, so an invoice's preimage is never revealed
/// for it.
/// </remarks>
internal static class FinalHopClaims
{
    ...
    /// </summary>
    public static byte[]? AcceptedPreimage(HtlcRecord? record) =>
        record is { Direction: HtlcDirection.Incoming, KnownPreimage: { } known }
