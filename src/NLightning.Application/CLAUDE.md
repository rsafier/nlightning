...
- `Node/Managers/PeerManager.cs`: `Dictionary<CompactPubKey, PeerModel>`. Sends handler replies. Maps exceptions, always scoped to the message's channel id (never an all-zero `error`, BOLT 1): ChannelErrorException sends `error` for the channel and disconnects, ChannelWarningException sends a `warning` and stays connected, any other exception (our bug) sends a channel `warning` and disconnects.
