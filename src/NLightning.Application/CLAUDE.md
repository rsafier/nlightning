...
- `Channels/Services/CommitmentSigningService.cs` (BOLT2 plan N3-T5): implements the Domain ports `Channels/Interfaces/ICommitmentSigner.SignRemoteCommitment(channel, CommitmentTxSpec, remoteNumber, remotePoint)` and `ICommitmentVerifier.VerifyLocalCommitment(channel, CommitmentTxSpec, localNumber, sig, htlcSigs)`, both returning `CommitmentTxSignatures(txid, sig, htlcSigs)` (not the N4 engine's ports in `Channels/Commitments/Interfaces`, NL-230). It runs factory (spec-driven overload) -> `BuildWithOutputMap` -> `HtlcTransactionModelFactory` -> `IHtlcTransactionBuilder.Build` -> `ILightningSigner` (`SignChannelTransaction` + `SignRemoteHtlcTransactions`, or `ValidateSignature` + `ValidateLocalHtlcSignatures`), so HTLC signatures are always in commitment output order. Singleton in `AddApplicationServices` (one instance behind both interfaces); nothing calls it yet (the N4 engine / N6 handlers will). Vectors: `test/NLightning.Integration.Tests/BOLT3/CommitmentSigningServiceVectorTests` (Appendix C/F: as node B the signatures equal `remote_signature`/`remote_htlc_signature`; as node A the vector signatures verify).
