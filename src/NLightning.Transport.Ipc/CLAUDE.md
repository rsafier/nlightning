...
- `Requests/*IpcRequest.cs` and `Responses/*IpcResponse.cs`: one pair per `ClientCommand`. Some have `ToClientRequest()` / `static FromClientResponse()` mappers to `Domain.Client.Requests|Responses`. `ListChannelsIpcResponse` carries nested `ChannelInfoIpcResponse` rows (ClientCommand 8).
