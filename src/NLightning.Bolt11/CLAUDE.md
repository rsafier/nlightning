...
- `Services/InvoiceValidationService.cs`: `ValidateInvoice` (decode and encode) requires p and s, and exactly one of d/h. `ValidateForEncoding` (encode only) adds `ValidateFeatures`: `9` present with var_onion_optin and payment_secret, every BOLT 9 dependency set, and no known feature whose BOLT 9 context excludes invoices (unknown bits are not checked). Used statically by `Invoice` (`s_invoiceValidationService`); not registered in DI.
