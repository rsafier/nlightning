...
## Layout
- `DependencyInjection.cs` — `AddSerializationInfrastructureServices()` registers 8 singletons (including `IHopPayloadSerializer`). Does NOT register `ITlvConverterFactory` (comes from `AddBitcoinInfrastructure`, `src/NLightning.Infrastructure.Bitcoin/DependencyInjection.cs:36`).
