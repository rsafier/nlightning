...
## Layout
- `DependencyInjection.cs` — `AddSerializationInfrastructureServices()` registers 9 singletons (including `IHopPayloadSerializer` and `IFailureMessageSerializer`), plus `TryAddSingleton<ITlvConverterFactory, TlvConverterFactory>` (also TryAdded by `AddInfrastructureServices`), so this layer resolves on its own.
...
- `Onion/FailureMessageSerializer.cs` — `IFailureMessageSerializer` (Domain): BOLT 4 failuremsg <-> `FailureMessage` and return-packet framing (see Onion section).
...
- `closing_signed` `fee_range` (NL-198) and `commitment_signed` `funding_txid` (TLV 1, `FundingTxIdTlv`, NL-199) are optional on receipt (nullable on the message); `IMessageFactory.CreateCommitmentSignedMessage` always sets `funding_txid` (the sender MUST). Both serializers read strictly with known set {1} and wrap a converter's `InvalidCastException` (bad TLV length) in `MessageSerializationException`.
...
- `Onion/FailureMessageSerializer.cs` (`IFailureMessageSerializer`, synchronous because `FailureOnionService` calls it inside its crypto loop): `u16 code || data || tlv_stream`. The data length per code comes from `FailureMessage.TryGetDataLength`; a truncated known code throws `SerializationException` (`TryDeserialize` returns false). Bytes after the data are read as a BOLT 1 TLV stream (canonical bigsize, increasing types, in-bounds lengths, no even types since no failure TLVs are known); anything else is ignored (`Extension = null`), because the origin MUST ignore extra bytes. Extension records are written verbatim from `BaseTlv.Value`. `SerializeErrorPayload(message, minFailurePadLength = 256)` writes `failure_len || failuremsg || pad_len || zero pad` with `failure_len + pad_len >= min` (min < 256 throws; the whole packet incl. HMAC must fit in 32768). `TryReadErrorPayload` checks only `failure_len`, not the padding.
