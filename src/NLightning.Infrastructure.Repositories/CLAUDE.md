...
- There are no commitment-number columns yet (they come with the N1-T5 migration). `MapEntityToDomain` sets `ChannelModel.LocalCommitmentNumber`/`RemoteCommitmentNumber` from `LocalRevocationNumber`/`RemoteRevocationNumber` (current number = revocations of that side, exact in steady state); nothing advances them yet (NL-188).
