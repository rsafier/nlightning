...
- There are no unit tests for this project. Real coverage comes only from the Docker e2e tests (`AbcNetworkTests.cs`, `ChannelOpeningFlowTests.cs`), which configure `Database:Provider=Sqlite` and run `context.Database.Migrate()`: `dotnet test test/NLightning.Integration.Tests --filter "FullyQualifiedName~Docker"`. The provider smoke tests in `test/NLightning.Integration.Tests/Docker/{Sqlite,Postgres,SqlServer}Tests.cs` are commented out. The best new test is a Sqlite `:memory:` round-trip with MigrationsAssembly `NLightning.Infrastructure.Persistence.Sqlite`.

## Gotchas
- Postgres uses snake_case (`UseSnakeCaseNamingConvention`) and ALWAYS calls `EnableSensitiveDataLogging()`, whatever the config says. Sqlite and SqlServer use PascalCase. Raw SQL must handle both.
- SQL Server bug: `ChannelEntity.RemoteNodeId` (33-byte pubkey) is mapped `varbinary(TransactionConstants.TxIdLength)` = 32 (`EntityConfiguration/Channel/ChannelEntityConfiguration.cs:83`). The fix is `CryptoConstants.CompactPubkeyLen`, and it needs a SqlServer migration.
...
- `State`/`Direction` on `ChannelEntity`/`HtlcEntity` are raw `byte`. Compare with `== (byte)Enum.X`, not `.Equals(Enum.X)` (boxed enum never equals a byte). Known bug: `src/NLightning.Infrastructure.Repositories/Database/Channel/ChannelDbRepository.cs` ~line 201 uses `h.State.Equals(HtlcState.Offered)` / `htlc.Direction.Equals(HtlcDirection.Outgoing)`, so no HTLCs are ever mapped back.
...
- `HtlcEntity.Signature` exists and is read in `HtlcDbRepository` (mapping to domain) but never set when building the entity (`HtlcDbRepository.cs` ~line 81), so it is always null.
