...
public interface ICommitmentTransactionModelFactory
{
    ...
    /// other remote number throws <see cref="InvalidOperationException"/>, so use the <see cref="CommitmentSpec"/>
    /// overload with an explicit point for it. The balances and HTLCs come from
    /// <see cref="CommitmentSpec.FromChannel"/> (gross channel balances made net).
    /// </param>
    ...
    /// <summary>
    /// Creates a commitment transaction from an explicit <see cref="CommitmentSpec"/> (BOLT 3 "Commitment Transaction
    ...
    /// the signer.
    /// </param>
    CommitmentTransactionModel CreateCommitmentTransactionModel(ChannelModel channel, CommitmentSpec spec,
