...
- `FeatureSet.DeserializeFromBytes` copies its input (it used to reverse the caller's array in place, which also scrambled `ChannelTypeTlv.Value` for multi-byte channel types). `new FeatureSet()` sets 5 compulsory bits: data_loss_protect, var_onion_optin, static_remote_key, payment_secret, channel_type.
