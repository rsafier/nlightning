...
- `FeatureOptions` defaults `OptionRouteBlinding` and `OptionAttributionData` to No; turn them on only when M5/M3b land. Other unimplemented features (gossip_queries_ex, basic_mpp, dual_fund, quiesce, provide_storage, scid_alias) also default to No; data_loss_protect is advertised Optional (not Compulsory) and gossip_queries stays Optional (see NL-100).
