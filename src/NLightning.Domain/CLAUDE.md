...
- `CommitmentNumber(openerPaymentBasepoint, accepterPaymentBasepoint, ...)`: BOLT 3 obscuring is SHA256(opener || accepter), so pass the funder's basepoint first regardless of which side is local (`ChannelDbRepository` reload still passes local first, NL-127). It is immutable: `Increment()` returns a new instance; store it with `ChannelModel.UpdateCommitmentNumber`.
