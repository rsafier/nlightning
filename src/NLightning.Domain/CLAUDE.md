...
4. For new TLVs: add a `TlvConstants` entry, add `Tlv/XTlv.cs : BaseTlv`, add a converter in `src/NLightning.Infrastructure/Protocol/Tlv/Converters`, register it in `TlvConverterFactory`, and add a case to the switch in `Infrastructure.Serialization/Tlv/TlvStreamSerializer.SerializeAsync`.
