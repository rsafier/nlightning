...
- `Converters/EndianBitConverter.cs` — big- and little-endian byte helpers (its trim/pad is not truncated-int compliant). `Exceptions/` — Message/PayloadSerializationException, InvalidMessageException, ConnectionTimeoutException.
...
- `ChannelErrorException` with a channel id means send `error` for that channel and disconnect. Any other `ErrorException` (no channel id) is sent as a `warning` then disconnect, never as an all-zero `error` (BOLT 1: that fails every channel). `WarningException` means send `warning`. `ConnectionException` is local only (disconnect, nothing sent).
...
- `PingPongService.StartPingAsync` raises `DisconnectEvent` (ConnectionException) when no pong arrives within `NodeOptions.NetworkTimeout`, and returns quietly on shutdown. `PeerCommunicationService.HandlePingPongDisconnect` disconnects on a separate task, because `Disconnect` waits (up to 5 s) for the ping loop that raised the event. The ping loop only starts once our init is sent AND the peer's init is received (`TryStartPingPongService`); starting it earlier makes the unsent first ping time out. `PeerCommunicationService.Disconnect` is idempotent (`_disconnecting` flag, also set by `Dispose`), so `DisconnectEvent` fires at most once. Note `ConnectionException` derives from `ErrorException`, so `RaiseException` disconnects on it too (but sends no error message); that is how a malformed message (warning already sent by `MessageService`) closes the connection. `Disconnect` sends the error/warning, stops the ping loop, then disposes the `MessageService` (closing the socket) on a separate task before raising `DisconnectEvent`: `Disconnect` often runs on the transport read loop (e.g. an init rejection), and `TransportService.Dispose` waits up to 5 s for that loop to end.
...
- `PeerService.HandleMessage` dispatches IChannelMessage/error/warning; BOLT 7 `GossipMessage`s (raw, unparsed) are dropped with a debug log; `stfu` gets a channel-scoped `warning` and a disconnect (quiescence unsupported; BOLT 2 says only a disconnection ends quiescence, the channel is not failed); a non-init first message closes the connection without sending anything (BOLT 1); anything else (e.g. onion_message) is silently dropped.
- `Argon2Id` memory limits are in **bytes**: `DefaultMemLimit` = 64 MiB, `LegacyMemLimit` = 64 KiB (v1 key files only); the salt must be exactly 16 bytes. Key files (`Node/Models/KeyFileData.cs`) are versioned: v1 (no `version`, fixed salt, zero nonce, 64 KiB) is still readable and `SecureKeyManager.FromFilePath` rewrites it as v2 (random salt + nonce + Argon2 params stored in the file). Never change the v1 read path. Passwords are hashed as their full UTF-8 bytes on every backend (`ICryptoProvider.DeriveKeyFromPasswordUsingArgon2I` takes bytes); libsodium used to get the UTF-16 char count as the length, which truncated non-ASCII passwords, and `SecureKeyManager` still retries that truncated encoding when reading.
...
- `Protocol/Tlv/Converters/Onion/*TlvConverter.cs`: one converter per hop-payload TLV (2, 4, 6, 8, 10, 12, 16, 18), registered in `TlvConverterFactory`. Type numbers are in `src/NLightning.Domain/Protocol/Onion/Constants/OnionPayloadTlvTypes.cs`, not `TlvConstants`. Truncated ints go through Domain `Protocol/Tlv/TruncatedInt` (the single BOLT 1 tu16/tu32/tu64 implementation, also used by the Domain onion TLV constructors). `CurrentPathKeyTlvConverter` checks only length/prefix, not that the point is on the curve (M5 must validate it via `ISecp256K1Math`).
