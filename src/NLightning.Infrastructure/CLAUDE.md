...
- `PeerService.HandleMessage` dispatches IChannelMessage/error/warning; `query_channel_range` / `query_short_channel_ids` are answered through `Node/Services/GossipQueryResponder` (we keep no graph: one final empty `reply_channel_range`, and `reply_short_channel_ids_end` with `full_information` = 0; a malformed scid query gets a warning); `gossip_timestamp_filter` is ignored; other BOLT 7 messages (raw `GossipMessage`s and replies) are dropped with a debug log; `stfu` gets a channel-scoped `warning` and a disconnect (quiescence unsupported; BOLT 2 says only a disconnection ends quiescence, the channel is not failed); a non-init first message closes the connection without sending anything (BOLT 1); anything else (e.g. onion_message) is silently dropped.
