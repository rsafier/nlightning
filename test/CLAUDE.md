...
- `NLightning.Infrastructure.Tests`: crypto providers (`#if CRYPTO_LIBSODIUM` / `#if CRYPTO_NATIVE`), transport (BOLT 8 states/services), TLV converters, MessageService, PeerService (init handshake/routing), PeerCommunicationService (ping/pong, init timeout), PeerAddress.
...
- BOLT 3 state: `Bolt3IntegrationTests` asserts the Appendix B funding tx byte-for-byte and every Appendix C commitment tx (`AssertUnsignedTxEquals`, witness stripped) plus signatures. `Bolt3AppendixCVectors.ExpectedCommitTx1Htlc*Tx` hold the second-stage HTLC txs for commit tx 1, but the builder test is skipped for NL-056 (not implemented). Appendix F (anchors) tests exist and are skipped for NL-061 (only the funder's to_local amount differs); unskip them when fixing NL-061.
