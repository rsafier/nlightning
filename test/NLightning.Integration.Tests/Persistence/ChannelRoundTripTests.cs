...
public class ChannelRoundTripTests
{
                                                  ...
                                                  LightningMoney? remoteBalance = null)
    {
        ...
        // reload rebuilds the number from LocalRevocationNumber + 1.
        const ulong localRevocationNumber = 4;
        var sha256 = new Sha256();
        var commitmentNumber = isInitiator
                                   ? new CommitmentNumber(s_key3, s_key6, sha256, localRevocationNumber + 1)
                                   : new CommitmentNumber(s_key6, s_key3, sha256, localRevocationNumber + 1);
    ...
    private static void AssertChannelsEqual(ChannelModel expected, ChannelModel actual)
    {
        ...
        Assert.Equal(expected.CommitmentNumber!.Value, actual.CommitmentNumber.Value);
