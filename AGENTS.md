...
- Solution config / test project check (CI step): `python3 scripts/check-sln-configs.py`

Known: `Release.Wasm` builds only on linux-x64; tests in the `Docker` namespace need a Docker daemon.
