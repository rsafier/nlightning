...
- Status after gossip wave G-D (`wip/fafo` @ `48a8951`): no milestone changed. Payments and forwarding through the M4 switch and `PaymentService` are on for every network by default since BOLT 5 O6-T4 (`Node:EnableHtlcs ?? true`, NL-094 fixed); the ABCD suite stayed green (3 x 10/10). Graph routes (BOLT 7 G4, gossip wave G-C) are unchanged; mainnet gossip sync and relay stay off (BOLT7 D12). Open: M5 route blinding (NL-079, NL-339), `OptionAttributionData` experimental (NL-332).
- Status after ABCD wave 7 (`wip/fafo` @ `4c37998`): **M3b is done and wired; the feature stays experimental.** W7-A (migration owner, `AddAttributionData` on all three providers: `Htlcs.AttributionData`/`FulfillmentPayload`/`AddedAt`, `PaymentHops.HoldTimeMs`) carries attribution_data and fulfillment_payload through the engine, `IChannelOperations` (`FailHtlcAsync(AttributedErrorPacket)`, `FulfillHtlcAsync(AttributedFulfillment, ...)`, `GetHoldTimeAsync` in 100 ms units since the add was received), the handlers, retransmission and `MessageFactory` (TLV 1 and 3; NL-022 fixed), enforces the 32 KiB fulfillment_payload MUST after committing a valid preimage (NL-325 fixed), and verifies at the origin in `PaymentService` (blamed hop, hold times recorded on the fulfilled part's route) (3a54e11, 06b906c, e64da4e). The integrator wired `HtlcSwitch` to the seam (create as erring/final node, wrap as intermediate, forwarded fulfills) only when we advertise `OptionAttributionData` and the incoming add had no `path_key` (672ed61; NL-326 fixed). Proofs: `AttributionHarnessTests`; Docker `AttributionFlowTests` 5/5 (LND 0.20 accepts TLV 1 but does not implement the feature; three NLightning nodes with the feature on record both hops' hold times through the production switch, 4c37998). `OptionAttributionData` stays in `ExperimentalFeatures` (no LND interop proof possible, NL-332). M4: the HTLC-set commitment point is the invoice's Settled save, set membership is the incoming `HtlcRecord.KnownPreimage`, duplicates for a Settled invoice get 0x400F, and final-hop HTLCs of closing channels are claimed on chain (W7-B: 7ca5c60, db00321, 72e7f49, a3cf0ce; NL-316, NL-322, NL-323 fixed). The replay set is pruned on every block by `OnionReplayBlockPruner` (W7-C aa41f2b, 368a057; wired 672ed61; NL-327 fixed). Open: NL-332, NL-333 (retry policy ignores the attribution blame), NL-334 (reverted fulfill loses attribution on replay), NL-339 (blinded forward drops fulfillment_payload, M5), NL-321, M5.
- Status after ABCD wave 6 (`wip/fafo` @ `3ce3cad`): **M4 extended, M3b library done.** The replay set is persistent (W6-A, NL-078 fixed: `PersistentOnionReplayStore` over `OnionReplayEntries`, migration `AddOnionReplaySet`, keyed by HMAC, owned by the incoming HTLC, pruned past its cltv_expiry; `IOnionReplayCache` removed; `IncomingOnionProcessor.ProcessAsync(..., OnionReplayOwner? replayOwner, ...)` replaces `checkReplay`; a2b57ff, 53c038c). `basic_mpp` receive (W6-B, NL-081 fixed: `Switch/HtlcSet`, `mpp_timeout` after `Node:Switch:MppTimeout` 60 s, whole-set failure on a `total_msat` mismatch, advertised Optional and in our invoices; Docker `MppFlowTests`; ba3db36, 451aa79, 78a3beb). Payment retries, per-call fee limit and MPP split send (W6-C, NL-270 fixed: `PaymentRoutePlanner`, `PaymentRetryPolicy`, `PayInvoiceOptions`; Docker `PaymentRetryFlowTests`; 7024f57, 703d83b, 74c9014). M3b (W6-D, NL-072/NL-022 partial): `AttributionDataTlv`/`FulfillmentPayloadTlv` on the wire, strict (6d7e480), and `IAttributionDataService` create/wrap/verify for fail and fulfill, byte-exact at every hop against both inline traces (d5866a6), registered by `AddBitcoinInfrastructure` (9692ba4) but not used by the switch or send paths; `OptionAttributionData` stays experimental (seam and persistence: NL-326). The ABCD suite stayed green (3 × 10/10 on net10.0 and net11.0). Open: M3b switch seam (NL-326, NL-325), M5 route blinding, NL-321..NL-323, NL-327.
...
All of M3-T1..T3 are done; M3b has its wire and crypto library (ABCD wave 6, 6d7e480, d5866a6) and is persisted and wired into the channel layer, the switch and the send path (ABCD wave 7, NL-326 fixed); `OptionAttributionData` stays experimental (NL-332). For the as-built files and deviations, see "M3 as built" below.
...
| **M3b (optional)** attribution_data (**DONE** in waves 6-7; feature experimental, NL-332) | `UpdateFailHtlcMessage` TLV 1 (`AttributionDataTlv`, 920 bytes) + converter; extend `FailureOnionService` | Inline BOLT 4 "Returning Errors" trace (incorrect_or_unknown_payment_details, htlc_msat=100, height=800000, TLV 34001). Gate on `Feature.OptionAttributionData`. Until done, **set its default to No** in `FeatureOptions`. **Library done** (W6-D: TLVs 6d7e480; `IAttributionDataService` d5866a6, byte-exact against the Returning Errors and Returning success traces at every hop; the traces put hold time 1 on the erring/final node). Open: switch/send use and persistence (NL-326), the 32 KiB fulfillment_payload check (NL-325) |
...
Remaining:
- **M3b** attribution_data + `fulfillment_payload` (NL-072, NL-022): library done in ABCD wave 6 (`ammagext` confirmed by the byte-exact traces); persistence, channel layer, switch and origin verification done in wave 7 (NL-326, NL-022, NL-325 fixed). Keep `OptionAttributionData` experimental-gated until an interop proof beyond NLightning exists (NL-332); follow-ups NL-333, NL-334, NL-339.
...
| **M4-T3** Final hop. **Processor done** (`FinalHopProcessor`, 6156173, 234607e; no MPP; 0x0013/0x0012 are reported before 0x400F, as LND and CLN do); accept must be atomic in the switch (NL-253). **Done** (ca87313, d1476a4: accept and settle staged in the fulfill's save). **MPP done** (W6-B, ba3db36, 78a3beb: `HtlcSet` held until `total_msat`, `mpp_timeout` 0x0017 after 60 s, whole-set failure on a `total_msat` mismatch; restart-safe without a schema change; Docker `MppFlowTests` 451aa79) | `FinalHopProcessor` + invoice store (uses `src/NLightning.Bolt11`; requires a ProjectReference that does not exist today) | Unknown hash, bad secret or low amount → 0x400F with (htlc_msat, height); mismatched cltv/amount → 0x0012/0x0013; MPP timeout 60 s → 0x0017 |
...
| BOLT 7 (channel_update, graph) | since gossip wave G-B we announce public channels and keep a validated graph (`GossipIngress`/`GraphStore`, IPC 17/18); a Domain `GraphPathfinder` exists but is not used by `PaymentService` yet (BOLT7 plan G4-T3); the switch still refuses the real SCID of a public channel with option_scid_alias Optional (NL-348); 258 is a typed `ChannelUpdateMessage`, signed/verified with the node key (e7b5269, 3ba2e4f), sent to and received from the channel peer since W1-E (b93dd05); queries get empty replies (NL-205); no gossip sync (G3, NL-099) | channel_update in failures (optional; len 0 allowed), pathfinding beyond direct channels / route hints |
