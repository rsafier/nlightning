...
| BOLT 7 (channel_update, graph) | 256/257 are typed and dropped, 259 is a channel message without a handler yet (gossip wave G-A; raw `GossipMessage` removed); a Domain `GraphPathfinder` exists but is not used by `PaymentService` yet (BOLT7 plan G4-T3); 258 is a typed `ChannelUpdateMessage`, signed/verified with the node key (e7b5269, 3ba2e4f), sent to and received from the channel peer since W1-E (b93dd05); queries get empty replies (NL-205); no graph (NL-099) | channel_update in failures (optional; len 0 allowed), pathfinding beyond direct channels / route hints |
