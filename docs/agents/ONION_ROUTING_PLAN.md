...
All of M3-T1..T3 are done; M3b has its wire and crypto library (ABCD wave 6, 6d7e480, d5866a6) but is not wired into the switch (NL-326). For the as-built files and deviations, see "M3 as built" below.
...
Remaining:
- **M3b** attribution_data + `fulfillment_payload` (NL-072, NL-022): library done in ABCD wave 6 (`ammagext` confirmed by the byte-exact traces); switch seam open (NL-326). Keep `OptionAttributionData` experimental-gated until then.
