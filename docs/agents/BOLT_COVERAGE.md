...
Snapshot date: 2026-09-25, refreshed the same day on `wip/fafo` @ `1a38360` after the fix swarm. It was compiled from per-area research maps. Every claim flagged "verified" was checked against the code while writing this file.
...
4. **All tests run in CI.** `dotnet test --filter 'FullyQualifiedName!~Docker'` runs 2146 tests (1 intentional skip) including Application.Tests (78) and Daemon.Tests (109); `scripts/check-sln-configs.py` keeps it that way (NL-167).
...
| shutdown (38), closing_signed (39) + fee_range | complete (wire) | `ShutdownMessage.cs`, `ClosingSignedMessage.cs`, `.../Tlv/FeeRangeTlv.cs` | `ShutdownMessageTests.cs`, `ClosingSignedMessageTests.cs` | closing_signed still requires the optional `fee_range` (NL-198). |
...
| Commitment tx (to_local / to_remote / HTLC outputs, obscured number, BIP69+CLTV ordering) | complete | `src/NLightning.Domain/Bitcoin/Transactions/Factories/CommitmentTransactionModelFactory.cs`, `src/NLightning.Infrastructure.Bitcoin/Builders/CommitmentTransactionBuilder.cs`, `Comparers/TransactionOutputComparer.cs`, `src/NLightning.Domain/Protocol/Models/CommitmentNumber.cs` | Every Appendix C commitment tx byte-for-byte in `Bolt3IntegrationTests.cs`, `CommitmentNumberTests.cs`, `CommitmentTransactionBuilderTests.cs` | Balances are gross and each HTLC is debited from its offerer; to_remote uses the holder's dust limit (NL-062). The factory throws when both outputs are below reserve (NL-196). |
| option_anchors | partial | `src/NLightning.Domain/Bitcoin/Transactions/Outputs/AnchorOutputInfo.cs`, `src/NLightning.Infrastructure.Bitcoin/Outputs/ToAnchorOutput.cs`, `src/NLightning.Domain/Bitcoin/Transactions/Factories/CommitmentTransactionModelFactory.cs` | 9-vector Appendix F theory in `Bolt3IntegrationTests.cs` | Commitment fee and both anchors match Appendix F (NL-061). HTLC trim fee still uses 666/706 instead of 0 (NL-195); no anchor HTLC txs. option_anchors is experimental-gated. |
...
| HTLC-success / HTLC-timeout second-stage txs | stub | `src/NLightning.Infrastructure.Bitcoin/Transactions/Htlc{Success,Timeout}Transaction.cs` (commented out), `Outputs/HtlcResolutionOutput.cs` | Appendix C HTLC-tx vectors wired, test skipped (NL-056) | `HtlcResolutionOutput` key order fixed (NL-058). |
...
| Per-commitment secret gen + shachain storage | complete (partial API) | `KeyDerivationService.cs`, `src/NLightning.Infrastructure/Protocol/Services/SecretStorageService.cs` | Appendix D vectors (`Bolt3IntegrationTests.cs`) | `GetBasepointPrivateKey` and `LoadFromIndex` throw `NotImplementedException` (NL-066). The shachain is not persisted (NL-136). Reload keeps the remote funding key and the opener/accepter basepoint order (NL-126, NL-127). |
...
**Overall: partial (M1+M2 of the plan done).** The primitives, onion packet, Sphinx construct/peel, hop payload parsing/validation and an in-memory replay cache exist and are checked against the official `bolt04/*.json` vectors. Nothing calls them yet: there is no HTLC handling, no error onion (M3), no forwarding/final-hop logic (M4), and no route-blinding payload processing (M5). The spec requirements, design and milestone plan are in [`docs/agents/ONION_ROUTING_PLAN.md`](ONION_ROUTING_PLAN.md).
...
| short_channel_id encoding | complete | `src/NLightning.Domain/Channels/ValueObjects/ShortChannelId.cs`, `BlockchainMonitorService.cs` | Masks fixed to the BOLT 7 widths (NL-101); the tx index is the position in the block, 24-bit end to end (NL-102). The channel's SCID is not persisted (NL-225). |
...
- Balances are stored as whole satoshis (NL-191); the channel's real SCID is not persisted (NL-225); the shachain is not persisted (NL-136).
...
5. **BOLT 2 normal operation (the largest block).** Add ChannelModel mutators and HTLC commitment-dance states. Add handlers plus `ChannelManager` cases for update_add_htlc, update_fulfill_htlc, update_fail_htlc, update_fail_malformed_htlc, commitment_signed, revoke_and_ack and update_fee. Add HTLC signing to `ILightningSigner`. Add HTLC-success and HTLC-timeout tx builders with the Appendix C HTLC vectors and Appendix F anchors. Add per-channel message serialization (locking). (The onion is already mandatory in `UpdateAddHtlcPayloadSerializer`.)
