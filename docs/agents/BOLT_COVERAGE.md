...
| Hop payload TLVs (2, 4, 6, 8, 10, 12, 16, 18) | partial (types only) | Domain types in `src/NLightning.Domain/Protocol/Onion/Tlv/`, type numbers in `.../Onion/Constants/OnionPayloadTlvTypes.cs`, converters in `src/NLightning.Infrastructure/Protocol/Tlv/Converters/Onion/` (registered in `TlvConverterFactory`) | `test/NLightning.Infrastructure.Tests/Protocol/Tlv/Converters/Onion/` | No hop-payload serializer or semantic validator yet (M2). `current_path_key` (12) is not curve-validated by its converter. |
