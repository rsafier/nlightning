...
5. Once both inits are exchanged, ping/pong keepalive starts (`src/NLightning.Infrastructure/Protocol/Services/PingPongService.cs`: random 30-300 s; NL-006). `IChannelMessage`s go to `OnChannelMessageReceived`, and error/warning to `OnAttentionMessageReceived`. Gossip 256-258 and replies are dropped (259 goes the channel path since gossip wave G-A), queries 261/263 get empty replies (`GossipQueryResponder`), `stfu` gets a channel `warning` + disconnect; anything else is dropped.
