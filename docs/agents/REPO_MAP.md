...
5. Once both inits are exchanged, ping/pong keepalive starts (`src/NLightning.Infrastructure/Protocol/Services/PingPongService.cs`: random 30-300 s; NL-006). `IChannelMessage`s go to `OnChannelMessageReceived`, and error/warning to `OnAttentionMessageReceived`. Since gossip wave G-B, 256/257/258 go to the optional `IGossipIngress` (the graph; 258 also to `ChannelUpdateService` through `OnChannelUpdateReceived`), `gossip_timestamp_filter(0, 0xFFFFFFFF)` is sent after our init to `gossip_queries` peers while the graph is enabled, the peer's `remote_addr` is kept only as `ObservedAddress` (NL-344), and 259 goes the channel path (`AnnouncementSignaturesMessageHandler`); query replies are dropped, queries 261/263 get empty replies (`GossipQueryResponder`), `stfu` gets a channel `warning` + disconnect; anything else is dropped.
