...
> Status: generated 2026-09-25 against `main` @ `e330fcf` (release v2.0.0 merge); refreshed 2026-09-25 on `wip/fafo` @ `1a38360` after the fix swarm (§1, §3-§5, §6.4, §8-§11); the per-BOLT status table in §5 was refreshed after ABCD wave 6 (`3ce3cad`), and its BOLT 7 row and the gossip dispatch notes after gossip wave G-C (`4dc0f77`); the intro paragraph below predates the ABCD waves. For current status prefer `BOLT_COVERAGE.md`. Bug status lives in [`ISSUES.md`](ISSUES.md), not here.
...
| `Client` | `ClientCommand` enum (0..19; next free 20), `ErrorCodes`, request/response models, `ClientException`, `INamedPipeIpcService` |
...
**Tests:** `test/NLightning.Infrastructure.Tests` (crypto, transport, TLV converters, MessageService, PeerAddress, `PeerServiceTests`/`PeerServiceLifecycleTests`, `PeerCommunicationServiceTests`/`...LifecycleTests`, `PeerServiceGossipTests`, interactive-tx validators). BOLT 8 vectors are in `test/NLightning.Integration.Tests/BOLT8`. TcpService has no unit tests.
...
**Tests:** `test/NLightning.Integration.Tests/Persistence/` (SQLite `:memory:` with the real migrations: `ChannelDbRepositoryTests`, `ChannelStateDbRepositoryTests` (50 simulator transitions, mid-dance reload, crash after every statement), `CommitmentStateMigrationRoundTrip`/`LegacyChannelMigrationRoundTrip` (seeded pre-migration rows, shared with the Docker provider tests), `UtxoDbRepositoryTests`, `BaseDbRepositoryTests`, `UnitOfWorkUtxoTests`, `FundingConfirmedAliasPersistenceTests`, `PersistenceConfigurationTests`, `SqlitePersistenceTests`). Docker `PostgresTests`/`SqlServerTests` run the same round trip against containers.
...
5. Once both inits are exchanged, ping/pong keepalive starts (`src/NLightning.Infrastructure/Protocol/Services/PingPongService.cs`: random 30-300 s; NL-006). `IChannelMessage`s go to `OnChannelMessageReceived`, and error/warning to `OnAttentionMessageReceived`. Since gossip wave G-B, 256/257/258 go to the optional `IGossipIngress` (the graph; 258 also to `ChannelUpdateService` through `OnChannelUpdateReceived`), the peer's `remote_addr` is kept only as `ObservedAddress` (NL-344), and 259 goes the channel path (`AnnouncementSignaturesMessageHandler`); since gossip wave G-C 261-265 go to the optional `IGossipSyncService` (`Application/Gossip/Sync/GossipSyncManager`: queries answered from the graph by `QueryResponder`, our own sync by range query → SCID batches → `gossip_timestamp_filter`, which replaced the G-B bootstrap filter `(0, 0xFFFFFFFF)`; without a sync service queries get empty replies), `stfu` gets a channel `warning` + disconnect; anything else is dropped.
...
2. `ChannelManager.HandleFundingConfirmationAsync` sets `FundingCreatedAtBlockHeight` and `ShortChannelId(height, txIndex, vout)`. The tx index is the block position (NL-101, NL-102). The handler persists the SCID with the channel (`Channels.ShortChannelId`, NL-225).
...
| 3 transactions | Done for non-anchor channels | Funding, commitment, HTLC-success/timeout, closing (legacy and simple) and sweep/penalty txs, every Appendix C/F vector byte-exact. Anchors on chain are BOLT 5 O7 (NL-314) |
...
| HTLC storage | `Infrastructure.Persistence/Entities/Channel/HtlcEntity.cs` (engine `HtlcRecord` fields: state 10-39, `OnionRoutingPacket`, `PathKey`, removal, `KnownPreimage`, `OnionSharedSecret`), written by `ChannelStateDbRepository` | Onion and shared secret persisted (N5); no forward link yet (NL-137) |
...
- **Stored bytes:** `ChannelStateDbRepository` stores the onion packet per HTLC, commitment HTLC sets as 53-byte records and the sent commitment_signed diff as wire bytes (`CommitmentStateEncoding`), so changing those formats changes the meaning of stored rows. Legacy HTLC rows (states 0-3) are refused per channel (NL-025).
