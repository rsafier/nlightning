...
- **Status (2026-09-26, gossip wave G-D, lane M4 `wip/fafo-g-d-m4-mainnet-htlc-gate-s1`):** **O6-T4 done: the mainnet gate is open.** `NodeOptions.HtlcsEnabled` is `Node:EnableHtlcs ?? true`, so HTLCs are on for every network including mainnet unless `Node:EnableHtlcs=false`. Evidence re-run on this branch before the flip (net10.0, in-container runner, `--network host`): the whole on-chain Docker suite **24/24 including both `Explicit` O5 by-hand variants**, and N9 `ChannelSafetyFlowTests` 2/2. NL-315 fixed on the way (the local resolver reads a peer's HTLC spend from its block, no txindex needed). The daemon's config template (lane D3) writes `"EnableHtlcs": null` for mainnet/testnet, which binds as unset and so follows this default (true for regtest/signets). **Kept by the G-D integrator** after the full Docker matrix on the integrated `wip/fafo` (see "O6-T4 integration record" in §5). Decision, evidence and the follow-up review: "O6-T4 decision" in §5. Still open: O7 anchors (NL-314, NL-067 second half), follow-ups NL-307..NL-309, NL-312, NL-313, NL-318, NL-329, NL-330, NL-335, NL-336 (none a mainnet fund-safety blocker).
- Status after gossip wave G-B (superseded by the line above; `wip/fafo` @ `5bbfbb5`): **the O6-T4 blockers are fixed** (lane M2): NL-337 (c4fab04), NL-320 (6ea8b1c, 7e96e28, 9b4e4c7) and NL-311 (eff0196, 1902bb5, e0b3421, e3023b8); the whole on-chain Docker suite (22 + 2 Explicit, incl. the new `OnchainWatchCatchUpTests`) is green at integration, so Proofs O3-O6 pass. **The gate itself is unchanged** (`Node:EnableHtlcs` unset = regtest only): the decision belongs to the G-D integrator after a re-run of N9 `ChannelSafetyFlowTests`, ABCD and the LND/CLN normal-operation suites (ABCD 3/3 and LND/CLN were green at the G-B integration). Evidence and remaining risks: "O6-T4 evaluation evidence" in §5. Still open: O6-T4 decision, O7 anchors (NL-314, NL-067 second half), follow-ups NL-307..NL-309, NL-312..NL-315, NL-318, NL-329, NL-330, NL-335, NL-336.
...
### O6-T4 decision (gossip wave G-D, lane m4-mainnet-htlc-gate, 2026-09-26)

**Decision: open the gate.** `NodeOptions.HtlcsEnabled` = `EnableHtlcs ?? true` (`src/NLightning.Domain/Node/Options/NodeOptions.cs`): HTLCs (payments and forwarding) are on for every network including mainnet when `Node:EnableHtlcs` is unset; `Node:EnableHtlcs=false` still refuses every HTLC on any network. Proofs O3-O6 (the O6-T4 criterion, BOLT2 §0.6) pass, the three named blockers (NL-311, NL-320, NL-337) were fixed in gossip wave G-B, and none of the remaining follow-ups (below) loses funds on a non-anchor channel. Options tests: `Domain.Tests/Node/Options/NodeOptionsTests` (`Given_AnyNetwork_When_EnableHtlcsUnset_Then_HtlcsEnabled` for mainnet/testnet/signet/regtest, `Given_DefaultOptions_When_Read_Then_MainnetWithHtlcsEnabled`, `Given_EnableHtlcsFalse_When_Read_Then_HtlcsDisabledOnEveryNetwork`), `Daemon.Tests/Extensions/NodeServiceExtensionsTests.Given_EnableHtlcsConfig_*` (mainnet/testnet unset → on, `false` → off).

**Config template (lane D3, integrated in the same wave):** `NodeConfigurationExtensions.CreateDefaultConfigJson` writes `"EnableHtlcs": true` for regtest and signets and `null` for mainnet and testnet; `null` binds as unset, so a node whose config was written by the template follows this code default (HTLCs on) and the operator's switch stays visible in the file (`NodeServiceExtensionsTests.Given_DefaultConfigJson_*` expects null there and checks that `HtlcsEnabled` equals the code default). Anchors stay off (`OptionAnchors` No until O7), and public channels on mainnet stay behind `Gossip:AllowPublicChannelsOnMainnet` (BOLT7 plan D12): a routing node needs both switches.

**Evidence run on this branch before the flip** (`wip/fafo-g-d-m4-mainnet-htlc-gate-s1`, Release, net10.0, host-built dll in `mcr.microsoft.com/dotnet/sdk:10.0` with `--network host`, one process per run):
- On-chain suite, `-namespace NLightning.Integration.Tests.Docker.Onchain -explicit on`, first run on `wip/fafo` @ `4dc0f77`: 24 tests, **23 passed, 1 failed**: the `Explicit` `OnchainO5Tests.Given_NLightningCheaterBreach_When_ResolverDrivenByTheTest_Then_BitcoindMinesThePenalty` (no penalty from the test-driven resolver). Cause: a stale test, not a product bug. Since O8 the victim's `MempoolReactor` publishes the penalty while the revoked commitment is unconfirmed (log: "Unconfirmed Revoked ... waiting for it to confirm", "Penalty 81575ac4...: 2 input(s)"), both confirm in the same block and the resolver driven by the test finds every output already penalized. Fix (test only): that variant runs its victim with `WatchMempool = false`, as `OnchainO6Tests` (b) does. The LND by-hand variant passed as is.
- On-chain suite again after that fix and the NL-315 fix: **24/24 passed** (O0 smoke 2, O2 2, O3 4, O4 5, O5 4 incl. both `Explicit` by-hand variants, O6 3, `OnchainFinalHopTests` 1, `OnchainMempoolTests` 2, `OnchainWatchCatchUpTests` 1), 235 s.
- N9 `Docker/ChannelSafetyFlowTests` (`-class`): **2/2 passed** (HTLC deadline → our commitment broadcast and `OnchainResolving`; forward held downstream and failed back before the deadlines), 80 s.
- Non-Docker after the flip, Release, net10.0: 7008 tests, all pass, no skips (Domain 2392, Application 1538, Integration 648, Serialization 520, Infrastructure 401, Bitcoin 871, Bolt11 278, Daemon 360). Release and Release.Native build with the 5 baseline CS86xx warnings.
- Not re-run in this lane: the ABCD suite and the LND/CLN normal-operation suites (their Docker nodes run on regtest, where HTLCs were already on, so the flip does not change their behavior; they were green at the G-B and G-C integrations).

**O6-T4 integration record (gossip wave G-D integrator, 2026-09-26): the flip is kept.** The task's condition was that the whole on-chain suite (with both `Explicit` O5 variants), N9 `ChannelSafetyFlowTests`, ABCD three times in a row and the LND/CLN normal-operation suites are green in the integrator's run of the integrated branch (lanes D2, D1, M4, D3 cherry-picked onto `wip/fafo` @ `55c0abc`, plus the integrate commit wiring `GraphStore` into `GossipMetrics`). All were, Release, net10.0, host-built dll in `mcr.microsoft.com/dotnet/sdk:10.0` with `--network host`, one Docker process at a time:
- On-chain (`scripts/run-onchain.sh 1 Release -namespace NLightning.Integration.Tests.Docker.Onchain -explicit on`): **24/24** (22 regular + both `Explicit` O5 by-hand variants), 212 s.
- LND suite (`-namespace Docker`, `Docker.Utils`, `Docker.Mock`, without `SqlServerTests` and the server-database theory of `MultiNodeHarnessTests`, whose rows cannot be split by the runner): **58/58**, including `ChannelSafetyFlowTests` (N9) 2/2, `NormalOperationFlowTests`, `ReestablishFlowTests`, `CooperativeCloseFlowTests`, `FeeUpdateFlowTests`, `MppFlowTests`, `PaymentRetryFlowTests`, `AttributionFlowTests` and `PostgresTests`, 319 s.
- CLN interop (`-namespace Docker.Interop.Cln`): **22/22**, 250 s.
- ABCD (`scripts/run-abcd.sh 3`): **10/10 three runs in a row**.
- Gossip (`scripts/run-gossip.sh 3`, Proof G5 regression): **24/24 three runs in a row**.
- Non-Docker, net10.0: 7104 tests in Release and in Release.Native, all pass, no skips; the Long invariant simulator passes.

**Follow-up review (is it a mainnet fund-safety blocker?):**
- NL-307 (watcher mutates the model before its save): no. A failed save leaves memory at 37; the replayed block records the close; only the error's delivery is delayed.
- NL-308 (Unknown spend → OnchainResolving, not Failed): no. By design since NL-320; nothing of ours is recoverable from such a tx and every output is watched as for data loss.
- NL-309 (revoked state without a log entry): no. Only commitments revoked before O1 (wave 4) lack the log; HTLCs were off on mainnet until now, so no mainnet channel has HTLC-bearing revoked states without a log entry; to_local and to_remote are still penalized.
- NL-312 (Failed channel whose mutual close confirms stays Failed): no. A mutual close needs a cleared channel (no HTLC) and pays our shutdown script, a watched wallet address; the cost is a commitment rebroadcast that keeps failing and a channel left Failed (noise).
- NL-313 (catch-up scan fetches whole blocks): no, performance only.
- NL-314 (anchor HTLC outputs): no while `OptionAnchors` is not negotiated (O7). Operational consequence: on mainnet we only run `static_remotekey` channels, with peers that accept them.
- NL-315 (peer HTLC spend needs the tx from bitcoind): **treated as a blocker and fixed in this lane.** Without txindex, when the spend-time preimage stage was lost (failed save, crash, no mempool sighting), the local resolver could not read the peer's preimage claim of our offered HTLC, so a forwarded HTLC was neither fulfilled nor failed upstream and the upstream peer would time it out: a loss of the forwarded amount. `LocalCommitResolver.ReadSpendingWitnessAsync` now reads the spender from the block at the watch's recorded spend height first (as `RevokedCommitDataSource` does), then `getrawtransaction`; the unreadable-spender alert (`B5-LCL-LO-03`) is raised once per process and HTLC from the first round at or past the reasonable depth, so a node offline over that block is alerted too. Proof: `LocalCommitResolutionTests.Given_PeersPreimageClaimNotStagedAndNoTxIndex_When_ReasonablyDeep_Then_FulfilledFromTheSpendBlock` and `Given_UnreadableSpenderAndNodeOfflineAtTheReasonableDepth_When_NextRound_Then_AlertedOnce` (both fail without the fix). Residue: a pruned node that no longer has the block still gets only the alert (never a wrong fail).
- NL-318 (revoked resolver does not persist an on-chain preimage): no. The fulfill is raised at once and every round; after a restart it is rebuilt from the spend read from its block (no txindex needed), and the O8 mempool reaction stages the preimage from the unconfirmed claim. Only a pruned node that crashed between the spend and the upstream fulfill is exposed.
- NL-329 (funding tx reorged out for good keeps its SCID): no. Routing/invoice hygiene, not funds.
- NL-330 (upstream fails of a close replaced by a reorg): no for launch. It needs a reorg deeper than `ReasonableDepth` (6) that replaces the close with a different commitment carrying the same HTLC claimed with the preimage; a critical `[B5-GEN-06]` alert names the rows. Kept medium.
- NL-335 (invoice expired after lock-in not claimed on chain): no. The payer keeps its money (the HTLC times out back); we lose nothing we had.
- NL-336 (`DustExposureHtlcSwitch` can swallow the on-chain final-hop decision): no, same outcome as NL-335 (an over-the-dust-limit incoming final-hop HTLC times out back to the payer). Outside this lane's files (`Channels/Fees/`).

### O6-T4 evaluation evidence (gossip wave G-B, lane m2-gate-onchain, 2026-09-26)

This section records the evidence for the mainnet gate (the gate was opened in gossip wave G-D, see "O6-T4 decision" above). When written it did not open the gate: `NodeOptions.HtlcsEnabled` still defaults to regtest only (`Node:EnableHtlcs` unset, `NodeOptions.cs`), and the daemon template still writes the per-network value. The integrator of wave G-D records the decision.
...
| **O6-T4** Mainnet gate. **Done** (gossip wave G-D lane M4: default on for every network, `Node:EnableHtlcs=false` turns it off; evidence and decision in "O6-T4 decision"; earlier: opened 09052d0, reverted 0c0d5c8; NL-316, NL-322 fixed in wave 7; NL-311, NL-320, NL-337 in gossip wave G-B) | `NodeOptions.HtlcsEnabled` default true on all networks only after Proofs O3-O6 pass (BOLT2 §0.6) | options test |
