...
**Not changed by this lane:** the daemon's `appsettings.json` template (`NodeConfigurationExtensions.CreateDefaultConfigJson`, owned by lane D3 this wave) still writes `"EnableHtlcs": false` for mainnet and testnet, so a node whose config was written by the template keeps HTLCs off until the operator (or D3) sets it to true; the template's `ENABLE_HTLCS` should become `true` for every network (and `NodeServiceExtensionsTests.Given_DefaultConfigJson_*` then expects true for mainnet/testnet). Anchors stay off (`OptionAnchors` No until O7), and public channels on mainnet stay behind `Gossip:AllowPublicChannelsOnMainnet` (BOLT7 plan D12): a routing node needs both switches.
