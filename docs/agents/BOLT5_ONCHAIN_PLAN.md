...
| Task | Files | Acceptance |
|---|---|---|
| **O6-T1** Fee policy + `IFeeService` target. **Policy done** (369314d); scheduler and target estimate open (NL-317, NL-296) | Domain `Onchain/SweepFeePolicy.cs`; `IFeeService.GetFeeRatePerKwAsync(uint confTarget, ct)`; `SweepScheduler` bumps per block | policy table tests (deadline, caps, BIP125 increments); `SweepSchedulerTests.Given_NotConfirmedAfterInterval_Then_ReplacedWithHigherFee` |
