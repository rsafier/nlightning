...
- **Status (2026-09-26, `wip/fafo` @ `48a8951`, after gossip wave G-D):** no milestone changed. The HTLC feature gate (§0 item 6) is lifted: BOLT 5 O6-T4 is done, so `NodeOptions.HtlcsEnabled` = `Node:EnableHtlcs ?? true` on every network including mainnet, and `Node:EnableHtlcs=false` still refuses every HTLC (6de56ad, kept by the integrator in 48a8951 after the full Docker matrix: N9 `ChannelSafetyFlowTests`, `NormalOperationFlowTests`, `ReestablishFlowTests`, `CooperativeCloseFlowTests` and `FeeUpdateFlowTests` in the LND suite 58/58, CLN 22/22, ABCD 3 x 10/10; NL-094 fixed). Open BOLT 2 follow-ups unchanged (NL-279, NL-285, NL-286, NL-045).
- Status after ABCD wave 7 (superseded by the line above; `wip/fafo` @ `4c37998`): no milestone changed; wave 7 closed the M3b rows of §6.4 (B2-DEL-W01, B2-DEL-06, B2-DEL-R03 done; B2-DEL-07 partial, NL-339): update_fail_htlc/update_fulfill_htlc carry TLV 1 and 3 through the handlers, persistence (migration `AddAttributionData`), retransmission and `MessageFactory` (NL-022, NL-325, NL-326 fixed). An oversized fulfillment_payload fails the channel after a valid preimage is committed. `HtlcExpiryMonitor` still decides PreimageKnown for final-hop HTLCs from the invoice status (NL-337, N9 follow-up).
- Status after ABCD wave 6 (superseded by the line above): N0-N10 are **done** and **N11-T1/T2 (`option_simple_close`) are done** (W6-E, NL-020 fixed): `closing_complete`/`closing_sig` with strict `closing_tlvs` serializers (aa569c8), `Channels/Close/Simple/` closer-pays builder and the closer/closee/closing_sig rules, RBF through `closechannel` (b67e065), routed in `ChannelManager` with a simple-close spend accepted as a mutual close (a2331b8), Docker proof against LND 0.20 `--protocol.rbf-coop-close` (we close, LND closes, both RBF; 5b9ce68), review fixes (d8dd76c, 7ebc93c). The feature left `ExperimentalFeatures` but `OptionSimpleClose` defaults to **No** (it needs `BeyondSegwitShutdown`), so the legacy close stays the default. N11-T3 (anchors) waits for BOLT 5 O7. Next: NL-279, NL-285, NL-286, NL-045, NL-312, NL-325 (fulfillment_payload size check). See "ABCD wave 6 record" in §5.
...
2. A task is done only when all of these pass:
   ...
   - `dotnet test --no-build -c Release --filter 'FullyQualifiedName!~Docker'` (includes Application.Tests and Daemon.Tests since N0-T1);
...
6. **Feature gate:** HTLC operation stays behind `NodeOptions.EnableHtlcs` (new, default `false` except on regtest) until N9 can fail a channel by broadcasting, and mainnet stays off until the BOLT 5 plan (NL-094) can sweep. **Lifted in gossip wave G-D** (BOLT 5 O6-T4 done: HTLCs on by default on every network, `Node:EnableHtlcs=false` turns them off).
...
- **Signer (N3):** `ILightningSigner` / `LocalLightningSigner` take commitment numbers, guard secret release by the local commitment number (`AdvanceLocalCommitment` after persist, NL-189), and sign/verify HTLC txs (NL-057). `CommitmentSigningService` (Application) signs/verifies a whole commitment with ordered HTLC sigs over `CommitmentTxSpec`; since ABCD W0-A the `Engine*Port` adapters implement the engine's ports over it (NL-230 fixed, 2fa8cf4).
...
| G19 | Remote shachain memory-only; `LoadFromIndex` throws | `SecretStorageService.cs:160,166`; `ChannelKeySetModel.LastRevealedPerCommitmentSecret` keeps one secret | NL-136, NL-066 | N3-T4 | **done** (NL-136, NL-066: e3145a5, e7ca51e, 9dfafda); runtime save on every revoke_and_ack and load per use (a604dff) |
...
- **N9-T4 / NL-271 done** (d040654, 41f5fc2): the `LocalCommitment` broadcast row with its commitment number is saved with Failed + the error; `ChannelManager`'s `MustBroadcast` path prepares the failure under its own lock (`PrepareFailureUnderLockAsync`) and publishes after it (`CompleteFailureAsync`); `ChannelFailureService` no longer sets Closed when the commitment confirms (the funding spend moves the channel to `OnchainResolving`, state 37). `forceclosechannel` refuses a Closing channel (7f6ebd9). S1 is restored at registration (NL-297).
- **B2-RE-27** (handle a broadcast of any sent commitment) and the on-chain half of **B2-FWD-05** (fulfill upstream from an on-chain preimage) are done by the BOLT 5 watcher and resolvers.
- `OnchainResolving` channels re-send their error on reconnect and refuse messages and operations.
- ABCD suite green 3 runs in a row; the LND suite 48/48 and CLN 17/17 on net10.0 and net11.0.

Still open in BOLT 2: N11 (`option_simple_close`, NL-020), B2-SHUT-S08 (NL-279), R09 deviation (NL-285), a Docker restart while closing (NL-286), local upfront script (NL-045), a Failed channel whose signed mutual close confirms (NL-312), IPC disconnect (NL-152).

### ABCD wave 4 record (status 2026-09-26, `wip/fafo` @ `6b5d50e`)

Wave 4 was BOLT 5 groundwork (see `BOLT5_ONCHAIN_PLAN.md` "ABCD wave 4 record") plus the BOLT 2 follow-ups of lane W4-E and the fee fixes of W4-D (lane SHAs mapped through the `-x` footers):
- **N9 re-verified**: Docker `ChannelSafetyFlowTests` 2/2 and `FeeUpdateFlowTests` 3/3 on `wip/fafo` at integration (in-container runner, `--network host`, NL-276). The LND-funded `FeeUpdateFlowTests` case is no longer skipped: the fee estimate is converted sat/vB × 250 to sat/kw with a 253 floor (`FeeRateConverter`, NL-288; 3350020, 803df11, 1036dfe), one shared started `FeeService` serves every consumer (`AddFeeServices`, 803df11, 960cf05), and as fundee we accept any `open_channel` feerate from the relay floor (NL-289, 1036dfe; B2-FEE-R01).
- **N9-T4 / NL-271 partial** (eb38c26): `ChannelFailureService` saves the commitment's watch in the same save as Failed + the error, publishes after the lock, and takes a `StillApplies` precondition checked under the lock; txids logged in display order (NL-275). A handler's `MustBroadcast` failure is still persisted Failed by `ChannelManager` first (hub file).
- **N10-T3 closing timeouts done** (d8680cd, 8249044; NL-284, B2-CLS-03, B2-CLS-R04): `ClosingTimeoutMonitor` fails the channel through `IChannelFailureService` when our `closing_signed` goes unanswered (counted only on the link the channel is pinned to) or no overlapping `fee_range` follows in time. Close fee estimate read through `ClosingFeeEstimator` (singleton, bounded wait under the lock; 8096700, 8249044). Shutdown addresses reserved across concurrent closes (NL-280 partial; 10b39e7, 8249044).
- **Proof N10 re-verified**: Docker `CooperativeCloseFlowTests` 4/4 against LND on `wip/fafo` at integration; new `ClnCloseTests` (8096700, a38c999) close against CLN in both roles and prove the `fee_range` receive path (R03, R05, R06) with a peer that sends it. NL-034 (legacy close epic) closed.
- **NL-263 fixed** at integration (5cc32ba): the UTXO locks move to the real channel id before `UpgradeChannel`.
- ABCD suite `scripts/run-abcd.sh 3` green (3 × 10/10); CLN interop 15/15 + `ClnChannelSessionTests` 2/2.

Still open in BOLT 2: N11 (`option_simple_close`, NL-020), B2-SHUT-S08 (NL-279), R09 deviation (NL-285), a Docker restart while closing (NL-286), local upfront script (NL-045), the NL-271 remainder, IPC disconnect (NL-152).
...
- N7-T1 (4620895): `src/NLightning.Domain/Channels/Reestablish/{ReestablishPlanner,ReestablishPlan,ReestablishLocalState,PeerReestablish,IReestablishTracker}.cs`. X=0 fail and broadcast, data loss when Y>L (secret checked against our Y-1), RAA resend at Y=L-1, CS diff resend at X=R+1, channel_ready when X=1 and L=0, `tx_abort` for `next_funding` on v1, `LastSent` order. Tests: `AT/Channels/Reestablish/ReestablishPlannerTests` (named cases plus about 4.7k table cases against a spec-literal oracle; in Application.Tests, not Domain.Tests, to stay in the lane).
- N7-T2 (4ec83d3, 22c29ae): `IChannelManager.OnPeerConnectedAsync`/`OnPeerDisconnectedAsync`/`OnPeerConnectionChanged`, called by `PeerManager`. At connect: revert the peer's unsigned updates (Open), send `channel_reestablish` for V1FundingSigned/ReadyForThem/ReadyForUs/Open, return stored errors of Failed channels. `ReestablishTracker` keeps the exchange status (Awaiting/Sent/Reestablished) apart from an "opened here" flag. `ReestablishGatedLivenessProbe` gates every `IChannelOperations` call and scheduler signature; update/CS/RAA on a non-reestablished Open channel get warning + close; `Publish` drops 128-135 of a channel not reestablished on the current connection (B2-RE-07 race). After a reestablish: `MarkLinkUp`, pending-event replay after the lock, commit scheduled (NL-252).
- N7-T3 (4ec83d3): retransmits the stored `SentCommitDiff` (byte-identical via `SentCommitDiffCodec`) and a regenerated RAA (`CreateRevokeAndAck(L-1, L+1)`) in their `LastSent` order, then unsigned updates with their original ids, then channel_ready (first local alias or real scid, NL-260). Shutdown re-send is N10.
- N7-T4 (4ec83d3): data loss persists `DataLossDetected` in its own save, then `ChannelFailedException` (B2-RE-23, `MustBroadcast=false`); signing and offering are refused afterwards. The signer itself does not refuse broadcast signing (no broadcast path until N9-T4).
- N7-T5 (82c4c37): `ResumeStartupStateAsync` handles every stored state; V1FundingCreated moves on when its funding watch exists, otherwise it is persisted Stale (funder remember rule). Closing is registered but sends no reestablish (N10). Gaps: NL-258 (funding tx never rebroadcast), NL-259 (UTXO locks of a forgotten channel).
- N7-T6 (82c4c37): `AT/Channels/Reestablish/FunderRememberRuleTests.Given_FundingSigned_Then_PersistedBeforeBroadcast` (not `FundingSignedMessageHandlerTests`); NL-048 stays wontfix.
- N6-T3 completed (4ec83d3): the stored error is re-sent on every connection and a `ChannelFailedException` sends an error without disconnecting (`PeerOutbox.TryEnqueueError`, `IPeerService.SendErrorAsync`); NL-200 fixed, broadcast is N9-T4 (NL-094).
- NL-234 fixed (4ec83d3): `HandleChannelMessageAsync` returns `Task`.
- Proof N7: I11 harness (1ad14ce; drop at every message boundary, crash at every save, old-backup restore detects data loss, unacked add re-sent once) plus the lost-channel_ready theory (22c29ae); Docker `ReestablishFlowTests` (a) our restart, (b) LND restart through `RestartByAlias("alice")` with address-hold containers (30c1bb8, NL-262), (c) crash after our CS is persisted (a4e95d7). **Met** at `a5675cb` (Docker 47/47).

N8 is done (W2-B, W2-C, W2-D; integration f2f1ef6, a5675cb):
- N8-T2 (ca87313, d1476a4): `Payments/Switch/HtlcSwitch` replaces `LocalOnlyHtlcSwitch` via `AddHtlcSwitchServices`; the final hop runs under a per-payment-hash lock and the invoice is Settled in the fulfill's own save (`IChannelOperations.FulfillHtlcAsync(..., stageWithFulfill, ct)`, NL-253); incoming settled rows are pruned on the new `IncomingHtlcSettled` event (NL-256, NL-243).
- N8-T3 (6cb279f, 083a726, f2f1ef6): `Payments/Send/PaymentService` (direct or via route hint, per-hop secrets persisted before `OfferHtlcAsync(HtlcOrigin.Local)`, `FailureInterpreter` on failure, startup reconciliation of in-flight payments); `payinvoice`/`listpayments` work. Deviation: the plan's "payee must be a direct peer" is widened to one route hint (M4-T6); no per-call fee limit (NL-270).
- `OfferHtlcAsync` persists the `HtlcOrigin` in the add's save (4ca9b56, NL-250).
- Proof N8 (fdc80af, a5675cb): `NormalOperationFlowTests` `Given_LndPaysOurInvoice_Then_Settled`, `Given_TrimmedHtlc_When_LndPaysOurInvoice_Then_Settled`, `Given_LndInvoice_When_WePay_Then_PreimageReturned`, `Given_ConcurrentPaymentsBothWays_Then_AllSucceed`, `Given_InFlightRestart_Then_FulfillArrivesAfterReestablish`. **Met** at `a5675cb`. The LND-pays-us probe payments now set the MPP record on the last hop (a5675cb): the real final hop rejects a payload without `total_msat` as `invalid_onion_payload` (BOLT 4), and LND's BuildRoute won't attach a payment address for a node outside its graph.

Deviations recorded in wave 2:
- Plan §3.11 step 3: the reestablish secret is checked against our secret Y-1 (spec), not L-1.
- Channels past funding_signed but not Open send `channel_reestablish` at connect (BOLT 2 MUST); a reestablish that arrives after channel_ready on a channel opened on this connection is answered.
- Pending events are replayed twice after a reestablish (manager and `LinkUpReplayingPeerLivenessProbe`); harmless, the switch is idempotent (NL-264).
- Shared-file touches accepted: `IPeerService`/`PeerService`/`PeerOutbox` (W2-A, additive), `ChannelOperationsService`/`ChannelStateTransitionService`/`IChannelOperations` and the Domain events (W2-B, additive).

Carried into wave 3 / N9-N10: N9-T2 `HtlcExpiryMonitor` (a forwarding node without it can lose funds), N9-T4 fail-the-channel broadcast and signer refusal after data loss (NL-094), N10 close with Closing/Negotiating resumption and shutdown re-send (NL-034, NL-036, B2-RE-28), NL-258/NL-259 (funding rebroadcast, UTXO locks), NL-269 (unverified LND sync edge), NL-246, NL-251, NL-254.

### ABCD wave 1 record (status 2026-09-25, `wip/fafo` @ `342d22e`)

N6 is done (lane W1-A; lane SHAs mapped to `wip/fafo` by the `-x` footers):
- N6-T1 (a604dff; NL-244 1de15f9, per-call SHA-256 for preimages 1392489): seven handlers in `Application/Channels/Handlers/` through the scoped `ChannelStateTransitionService`: `ApplyAsync` + one save, then `UpdateCommitments`, then send; the RAA secret is revealed only after the save (I3); after CS or RAA we sign at once when changes are pending, persisting `SentCommitDiff` (`SentCommitDiffCodec`) and `LastSent`. The first snapshot is built at the first channel_ready (NL-232); the peer shachain is loaded per use and exported in the same save (NL-136); funding_created holds the temporary and real id locks (NL-235); startup keeps Failed channels and persists `RevertUncommitted`.
- N6-T2 (a02afa7, aac5f60, e5f7312): `ChannelOperationsService : IChannelOperations` (singleton; lock, preconditions incl. `NodeOptions.EnableHtlcs`, engine, one save, publish under the lock through `IChannelMessagePublisher`, schedule after release), `CommitScheduler` (10 ms debounce, never signs while `RemoteNextCommit` exists, D7), `LocalOnlyHtlcSwitch` (peel; BADONION → malformed; final hop → `incorrect_or_unknown_payment_details`, or `temporary_node_failure` while no height is known; non-final → `temporary_node_failure`), startup replay of `DerivePending` outside the lock, and per-connection link pinning (`IPeerLivenessProbe.IsAliveAsync/MarkLinkUp`).
- N6-T3 partial (a604dff, a02afa7): `ChannelFailedException` → Failed + `ErrorSent` persisted under the lock, error sent, later updates refused on both sides. Missing: re-send on reconnect (B2-RE-05) and error without disconnect (W2-A, owner of `PeerManager`).
- N6-T4 (f5315c0, a02afa7, e5f7312): `TwoNodeHarness` with the production handlers, operations and scheduler; I7 after every phase; crossing adds; local-only fail-back to 2/2; a peer that is away.
- N6-T5 (a388fc4): Docker `NormalOperationFlowTests.Given_LndPaysUs_When_LockedIn_Then_FailedBackAndChannelActive` (alice `BuildRoute` + `SendToRouteV2` with a random hash; LND reads `INCORRECT_OR_UNKNOWN_PAYMENT_DETAILS` at source index 1 with a height above 0; commitment numbers 2/2; channel Active; peer connected).

Deviations recorded in wave 1:
- A normal-operation message on a channel that is not Open gets warning + close rather than a channel error (B2-NO-02), because there is no fail-the-channel broadcast yet.
- The final hop fails with `incorrect_or_unknown_payment_details` (the plan said `temporary_node_failure` for every HTLC); non-final onions and an unknown height still get `temporary_node_failure`.
- Ping-before-commit is only a connection check (`ConnectedPeerLivenessProbe`, NL-251); `IPeerService.LastMessageReceivedAt` does not exist.
- The shachain is loaded from the DB on each revoke_and_ack instead of once at startup.
- `OfferHtlcAsync` validates `HtlcOrigin` but does not persist it yet (NL-250).
- NL-234 (return-and-raise) is unchanged; it moves to W2-A with `IChannelManager`.

Carried into N7 (W2-A): `IPeerLivenessProbe.MarkLinkUp` after reestablish and pending-event replay (NL-252); retransmit persisted unsigned updates and the stored `SentCommitDiff`; re-send `ErrorSent` (B2-RE-05); reuse `LoadRemoteShachainAsync`, `SentCommitDiffCodec`, `CreateRevokeAndAck`; `IsReestablished` flag for `listchannels`. Carried into N8 (W2-B/W2-C/W2-D): `HtlcSwitch` replaces `LocalOnlyHtlcSwitch` via `services.Replace`, atomic final-hop accept (NL-253), origin persistence (NL-250), pruning order (NL-243). Also open: NL-138, NL-246 (pre-wave-1 channels without a snapshot), NL-254 (dust policy option).

### ABCD wave 0 record (status 2026-09-25, `wip/fafo` @ `0b7e617`)

All five items of "Remaining before N5" below are done:
1. NL-230 fixed (2fa8cf4): one engine port family; `EngineCommitmentSignerPort`/`EngineCommitmentVerifierPort`/`EngineRevocationVerifierPort` over `CommitmentSigningService` (concrete, no interface) and `IPerCommitmentSecretVerifier`, registered by `AddCommitmentEngineServices()`. NL-231 fixed (192e212): `CommitmentFees` deleted, `CommitmentFeeCalculator` is the only BOLT 3 fee/trimming code. Proof: `AT/Channels/Services/EngineCommitmentPortsTwoNodeTests` (real `LocalLightningSigner` both sides, anchors and not, txid signed == txid verified at every commitment).
2. N5-T1/T2/T3 done (4472a8b, bb2731a, f2e1a4a, a8d1381, 79f7657): see the N5 table.
3. N4-T4 done (b166ea0): see the N4 table.
4. Also: guessed limits of `HasInferredParams` channels are carried as `CommitmentParams.HasInferredLimits` and not enforced on receive (7ba115f, NL-194 follow-up).

Deviations recorded in wave 0:
- `CommitmentsResult` is `(Next, Outbound, Transition, Events)`: the plan's `Persist` is still `Transition` (its fields were frozen for W0-B).
- `IHtlcSwitch` has one method, `HandleAsync(IChannelDomainEvent, ct)`; the switch pattern-matches the event type. An incoming HTLC reaching final (39) raises no event; add an `IncomingHtlcSettled` only if N6 needs it for pruning.
- §3.5: `Commitments` is keyed `(ChannelId, Slot)` with slots LocalCurrent/RemoteCurrent/RemoteNext instead of `(ChannelId, Side, Number)` + status; there is no txid column (the engine has none) and no `Channels.FeeratePerKw` (the `FeeUpdates` table holds it). `HtlcEntity` keeps `PaymentPreimage` for a fulfill removal and a separate `KnownPreimage`. `ErrorSent` and `DataLossDetected` columns were added now so W1-A/W2-A need no migration. `RevocationWatchEntity` is still unmapped (NL-095).
- `IChannelOperations` has no `ShutdownAsync` until N10; the circuit reference is `Payments/ValueObjects/HtlcOrigin` (Local(hash) | Forwarded(chan, htlcId)).

Carried into N6 (ABCD W1-A): the first snapshot must be created with `IChannelStateDbRepository.InitializeAsync` at funding_signed/channel_ready with both remote points, before `ChannelReadyMessageHandler` overwrites the key set's first point (NL-232 rest); per transition `ApplyAsync` + one `SaveChangesAsync`, then `ChannelModel.UpdateCommitments`, then send; shachain `Export` on RAA / `Load` at startup (NL-136); after a restart `RevertUncommitted` and persist its transition before reestablish; startup replay with `ChannelDomainEvents.DerivePending` per channel inside a try/catch (legacy states throw); the switch must be idempotent (IncomingHtlcLockedIn and OutgoingHtlcFulfilled are replayed until resolved/pruned); dust policy on reload (NL-242); pruning (NL-243); NL-138 (memory repo still returns the shared model). Plus item 6 of the list below.

### Remaining before N5 (status 2026-09-25, `wip/fafo` @ `3c625e1`; items 1-5 done in ABCD wave 0, see above)
...
| **N0-T5** Gossip parse-and-drop. Resolves NL-100. **Done** (156c375); 261/263/265 also get empty replies (9e17af2, NL-205). Gossip wave G-A replaced the raw types: 256/257 typed and dropped, 259 routed as a channel message (57bb15b, BOLT7 plan G0). Gossip wave G-B: 256/257/258 go to the graph ingress and 259 has its handler (`AnnouncementSignaturesMessageHandler`, f6e76c2, NL-342; BOLT7 plan G1/G2). | message/payload types for 256–259 capturing raw bodies; serializers in both factory dictionaries; `PeerService.HandleMessage` drops them with a debug log | serializer round trips; receiving 258 raises nothing |
...
| **N3-T5** `CommitmentSigningService`. **Done** (908bd32) over `CommitmentTxSpec`; engine ports implemented over it in ABCD W0-A (NL-230, 2fa8cf4). | `src/NLightning.Application/Channels/Services/CommitmentSigningService.cs` (`ICommitmentSigner`, `ICommitmentVerifier`) | `AT/…/CommitmentSigningServiceTests` with Appendix C data: remote commitment sig + ordered HTLC sigs equal the vectors |
...
| **N4-T4** Events. **Done** (b166ea0): `IChannelDomainEvent` + `IncomingHtlcLockedIn` (once, on the RAA that moves 33→34), `OutgoingHtlcFulfilled` (immediately, first time the preimage is learned), `OutgoingHtlcFailed` (only when the removal is irrevocable, state 19), `OutgoingHtlcSettled`; `CommitmentsResult.Events`; `ChannelDomainEvents.DerivePending` for the I8 startup replay (throws on legacy states); `IHtlcSwitch` port. The simulator checks the event invariants after every step (500 seeds + 10k Long). | `Channels/Commitments/Events/*` | `…Given_IncomingAdd_When_BothRevoked_Then_IncomingHtlcLockedInOnce`; `…Given_DownstreamFail_Then_OutgoingHtlcFailedOnlyWhenIrrevocable` |
...
| Task | Files | Acceptance |
|---|---|---|
| **N5-T1** Migration `AddCommitmentState` (§3.5). Resolves NL-025 (legacy rows rejected at startup); partial NL-095, NL-137. **Done** (4472a8b; NL-025, NL-232, NL-238; §3.5 deviations in the wave-0 record; NL-095 not advanced) | entities, configs, `NLightningDbContext`, 3 migrations | migrations apply on Postgres, SQL Server, SQLite |
| **N5-T2** `ChannelStateDbRepository.ApplyAsync`. Resolves NL-192, NL-138. **Done** (4472a8b, a8d1381): `IChannelStateDbRepository` (`InitializeAsync`, `ApplyAsync(next, transition, extras)`, `LoadAsync`, onion secret get/set, `PruneSettledHtlcsAsync`); `IHtlcDbRepository` deleted; legacy channels refused one at a time. NL-138 still open (memory repo returns the shared model) | `src/NLightning.Infrastructure.Repositories/Database/Channel/ChannelStateDbRepository.cs`, `IUnitOfWork`, Domain interface; `ChannelDbRepository.UpdateAsync` stops writing HTLCs; memory repo returns snapshots | SQLite: apply 50 random transitions from the simulator, reload, equal model; mid-dance (`RemoteNextCommit` present) reload identical |
| **N5-T3** Crash injection + durability. **Done** (bb2731a, f2e1a4a, 79f7657; NL-237): `SqliteDurabilityInterceptor`, crash after every statement of a save, seeded pre-migration rows on all three providers | `test/NLightning.Tests.Utils/Mocks/CrashingUnitOfWork.cs`; SQLite `synchronous=FULL` | throw at the k-th save → no partial rows (rollback verified on SQLite); Postgres/SQL Server via existing fixtures locally |

**Proof N5 (passes):** round-trip and crash tests green on SQLite (CI); Postgres/SQL Server container round trips 6/6 locally.

### N6: Application wiring — HTLC dance with LND (fail-back)
Prerequisite: ONION M3-T1 + create half of M3-T2: **met** (ONION M3 done, NL-070). Also NL-230 (engine ports over the real signer): **met** (2fa8cf4).

| Task | Files | Acceptance |
|---|---|---|
| **N6-T1** Handlers + cases for 128–135. Resolves NL-031 (core); covers ONION M4-T1. **Done** (a604dff) | `UpdateAddHtlc`, `UpdateFulfillHtlc`, `UpdateFailHtlc`, `UpdateFailMalformedHtlc`, `CommitmentSigned`, `RevokeAndAck`, `UpdateFee` handlers; `ChannelManager.cs` cases | `AT/Channels/Handlers/*Tests`: state guard (`Open` + reestablished), violation → exception type, `Given_PersistFails_Then_NoRevokeSent` |
| **N6-T2** Operations, scheduler, events, switch. **Contracts done** (ABCD W0-A b166ea0, W0-C 2ede2ee, 1390027: `IChannelOperations`, `IHtlcSwitch`, `IChannelDomainEvent`, `NodeOptions.EnableHtlcs`); implementations **done** (a02afa7, aac5f60, e5f7312; ping is a connection check, NL-251) | Domain `IChannelOperations`, `IHtlcSwitch`, `IChannelDomainEvent`; `ChannelOperationsService`, `CommitScheduler` (debounce, ping-before-commit via `IPeerService.LastMessageReceivedAt`), `LocalOnlyHtlcSwitch`, `NodeOptions.EnableHtlcs` | `AT/Channels/Services/*Tests`; event replay on startup (crash after RAA persist → event once on restart) |
| **N6-T3** Failed-channel path. Resolves NL-200. **Done** (contracts 2ede2ee, 1390027, 4472a8b; persisted and refused a604dff, a02afa7; re-send on reconnect and error without disconnect 4ec83d3; broadcast is N9-T4) | `ChannelFailedException`, `ChannelState.Failed = 35`, `PeerManager` (send `error`, persist `ErrorSent`) | error persisted and sent; updates refused afterwards |
| **N6-T4** In-process two-node harness. **Done** (f5315c0) | `AT/Channels/Harness/TwoNodeHarness.cs` (two `ChannelManager`s joined by in-memory outboxes, real builders/signer, fixed keys) | add → CS → RAA → CS → RAA; fulfill; fail; fee round; 30 HTLCs each way; txids identical at every step (I7) |
| **N6-T5** Docker `Given_LndPaysUs_When_LockedIn_Then_FailedBackAndChannelActive`. **Done** (a388fc4; final hop fails with `incorrect_or_unknown_payment_details`) | `NormalOperationFlowTests.cs` | alice pays a test invoice to us over the N1 channel; we lock in and fail back with `temporary_node_failure`; LND reports the failure; the channel stays `Active`; `ListChannels` shows local/remote commitment numbers = 2 |

**Proof N6:** harness + Docker fail-back test. **Met** at `342d22e` (Docker 29/29 on OrbStack).

### N7: Reestablish and disconnect
| Task | Files | Acceptance |
|---|---|---|
| **N7-T1** `ReestablishPlanner` (pure). **Done** (4620895; tests in `AT/Channels/Reestablish/`) | `src/NLightning.Domain/Channels/Reestablish/{ReestablishPlanner,ReestablishPlan}.cs` | `DT/Channels/Reestablish/ReestablishPlannerTests`: exhaustive table over (L, R, waiting?, last order) × (their next_commitment, next_revocation, secret), both RAA/CS orders, data loss, `next_funding` on v1 → `tx_abort` |
| **N7-T2** Lifecycle hooks + gating + revert. Resolves NL-035 (core). **Done** (4ec83d3, 22c29ae) | `PeerManager` (after `init` → `OnPeerConnectedAsync`; disconnect → `OnPeerDisconnectedAsync`), `IChannelManager`, `ReestablishService`, `ChannelReestablishMessageHandler`, outbox gate | `AT/Channels/Managers/ChannelManagerReconnectTests` (two channels → two reestablish; local add queued until synced; unacked peer add dropped, same id accepted once) |
| **N7-T3** Retransmission. **Done** (4ec83d3; I11 proof 1ad14ce; shutdown re-send is N10) | stored `SentCommitDiff`, RAA regeneration, `LastSentOrder`, unsigned local updates, `shutdown` re-send, `channel_ready` when both numbers are 1, `error` re-send for Failed channels | harness: disconnect at every message boundary of the dance and crash at every persist point → both sides converge with identical txids (I11) |
| **N7-T4** Data loss. Resolves NL-035 (data_loss_protect). **Done** (4ec83d3; signer broadcast refusal left to N9-T4) | `DataLossDetected` flag, signer refuses broadcast signing, `ListChannels` field | `…Given_PeerAhead_WithValidSecret_Then_NoBroadcast_And_ErrorSent` |
| **N7-T5** Startup for non-Open states. Partial NL-036. **Done for existing states** (82c4c37; ShuttingDown/Negotiating are N10) | `ChannelManager.RegisterExistingChannelAsync` | Failed/ShuttingDown/Negotiating channels resume their state on reconnect |
| **N7-T6** Funder remember rule. Resolves NL-048 (spec: funder SHOULD NOT remember before broadcast; current persist-at-funding_signed-then-broadcast is correct — close NL-048 as wontfix with this test). **Done** (82c4c37, as `AT/Channels/Reestablish/FunderRememberRuleTests`) | `FundingSignedMessageHandler` | `AT/…/FundingSignedMessageHandlerTests.Given_FundingSigned_Then_PersistedBeforeBroadcast` |

**Proof N7:** planner + harness + Docker `Given_Restart_Then_Reestablish`: (a) idle restart of NLightning (same DB and key manager) → reestablish both ways, LND `PendingChannels` empty, N6 fail-back flow works again; (b) LND restart likewise; (c) test hook kills us after the CS persist but before the RAA arrives → after restart LND retransmits and the dance completes. **Met** at `a5675cb` (`Docker/ReestablishFlowTests`, a4e95d7, 30c1bb8).
...
| **N9-T4** `ChannelFailureService`. Partial NL-094, NL-095. **Done** (36d2270, 06da54b, wired 983b2b4; watch in the Failed save eb38c26; broadcast row in the Failed save and the handler path d040654, 41f5fc2, NL-271; the peer's commitment detected by the BOLT 5 watcher, NL-272) | error + sign latest local commitment with both sigs + broadcast + `Failed`; refuses under `DataLossDetected`; the only broadcast path | `AT/…/ChannelFailureServiceTests.Given_Revoked_Then_OnlyLatestBroadcast`, `…Given_DataLoss_Then_Refused` |
...
| **N10-T3** Shutdown + negotiation + `CloseChannel`. Resolves NL-034 (legacy); partial NL-036, NL-152. **Done** (b38ce86, 6d81ecd, 9733937, 5d0aafc, 8e0e154; timeouts d8680cd, 8249044 in wave 4) | `ShutdownMessageHandler`, `ClosingSignedMessageHandler`, `ChannelCloseService`, `LegacyClosingNegotiator`, states 23/25/30, migration `AddShutdownState`, `ClientCommand.CloseChannel` | negotiator property tests (convergence ≤ 64 rounds, strictly-between, range overlap); handler tests per §6 SHUT/CLS rows |
...
|---|---|---|---|---|
| B2-NO-01 | Five-stage update lifecycle | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T3 | `DT/…/CommitmentsScenarioTests.Given_SpecDiagram_When_Steps1To9_Then_EachUpdateTraversesFiveStates` |
| B2-NO-02 | HTLCs only after both `channel_ready` | DONE (a604dff; not Open → warning + close, deviation) | N6-T1 | `AT/…/UpdateAddHtlcMessageHandlerTests.Given_ChannelNotOpen_When_Add_Then_ChannelError` |
...
| B2-FWD-05 | Fulfill incoming on outgoing fulfill / on-chain preimage | DONE (off-chain: upstream fulfilled at once on `OutgoingHtlcFulfilled`, ca87313, d1476a4; on-chain preimage through the BOLT 5 resolvers, 7d3a6b3, 037c04b, 202341b, e5a556d; Docker O3 (b), O4 (e)) | N4-T4 | `…Given_DownstreamFulfill_Then_UpstreamFulfillImmediately` [M4][B5] |
...
| B2-ADD-S01 | Payer affords fee on both commitments above reserve | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `DT/Channels/Validators/UpdateValidatorTests.Given_FunderAtReserve_When_SendAdd_Then_Rejected` |
| B2-ADD-S02 | Anchors: both anchors affordable | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_AnchorsFunder_When_OnlyOneAnchorAffordable_Then_Rejected` |
| B2-ADD-S03 | Fee-spike buffer | DONE (engine N4; wired N6, a604dff/a02afa7); both commitments | N4-T2 | `…Given_SpikeBufferViolated_Then_Refused` |
| B2-ADD-S04 | Non-payer doesn't starve payer | DONE (engine N4; wired N6, a604dff/a02afa7); both commitments | N4-T2 | `…Given_NonFunderAdd_When_FunderCantPayFee_Then_Refused` |
| B2-ADD-S05 | `amount_msat > 0` | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `DT/…/SendAddTests.Given_ZeroAmount_Then_Rejected` |
| B2-ADD-S06 | ≥ remote minimum | DONE (engine N4; wired N6, a604dff/a02afa7); per-side params NL-194 fixed | N1-T4, N4-T2 | `…Given_BelowRemoteMinimum_Then_Rejected` |
| B2-ADD-S07 | `cltv_expiry < 500000000` | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_TimestampCltv_Then_Rejected` |
| B2-ADD-S08 | ≤ remote `max_accepted_htlcs` | DONE (engine N4; wired N6, a604dff/a02afa7); 483 fill tested; per-side params NL-194 fixed | N1-T4, N4-T2 | `…Given_RemoteMaxAcceptedReached_Then_Rejected` |
| B2-ADD-S09 | ≤ remote in-flight | DONE (engine N4; wired N6, a604dff/a02afa7); per-side params NL-194 fixed | N1-T4, N4-T2 | `…Given_InFlightExceeded_Then_Rejected` |
...
| B2-ADD-R04 | `cltv_expiry ≥ 5e8` → fail | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_TimestampCltv_Then_Violation` |
| B2-ADD-R05 | Duplicate `payment_hash` allowed | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_DuplicateHash_Then_Accepted` |
| B2-ADD-R06 | Repeated id after reconnect ignored | DONE (4ec83d3; harness `ReestablishHarnessTests`, unacked add re-sent with the same id and accepted once, 1ad14ce) | N7-T2 | `DT/…/ReestablishTests.Given_UnackedAdd_When_Reconnect_Then_SameIdAcceptedOnce` |
| B2-ADD-R07 | Other id violations → MAY fail | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_IdGap_Then_Violation` |
| B2-ADD-R08 | Decrypt onion (path_key, hash as AD) | DONE (`HtlcSwitch` peels every locked-in HTLC through `IncomingOnionProcessor`, ca87313) | N8-T2 | ONION M4-T2 tests [M4] |
| B2-ADD-R09 | Onion failure → failure message | DONE (`HtlcSwitch`: malformed → `update_fail_malformed_htlc`, other failures → `update_fail_htlc`, ca87313) | N6-T2, N8-T2 | `AT/Payments/HtlcSwitchTests.Given_BadOnion_Then_FailMalformedAfterLockIn` [M3][M4] |
| B2-ADD-R10 | BOLT 4 payload reader rules | DONE (validator wired through `IncomingOnionProcessor` in `HtlcSwitch`, ca87313; a payload without `total_msat` → `invalid_onion_payload`, proven in Docker a5675cb) | N8-T2 | `AT/Payments/FinalHopHtlcSwitchTests` [M4] |

### 6.4 Removing HTLCs
| ID | Requirement | Status | Task | Test |
|---|---|---|---|---|
| B2-DEL-W01 | fulfill TLVs 1/3, fail TLV 1 | DONE (6d7e480 wire; 3a54e11 factory, handlers, persistence, retransmission; NL-022) | ONION M3b | `ST/Messages/UpdateFulfillHtlcMessageTests`, `AT/Channels/Handlers/AttributionHandlerTests` [M3b] |
...
| B2-DEL-06 | `attribution_data` when negotiated | DONE (3a54e11, 672ed61; `HtlcSwitch` adds it when `OptionAttributionData` is advertised and the add had no `path_key`; the feature stays default No and experimental, NL-332) | N0-T4 (default No), M3b | `AT/Payments/Send/AttributionHarnessTests`, `AT/Payments/Switch/HtlcSwitchTests`, Docker `AttributionFlowTests` [M3b] |
| B2-DEL-07 | Relay `fulfillment_payload` | PARTIAL (forwarded fulfills wrap it, 672ed61; a blinded forward drops it, NL-339) | ONION M3b, M5 | `AT/Payments/Switch/HtlcSwitchTests`, Docker `AttributionFlowTests` [M3b] |
| B2-DEL-R01 | Unknown id → fail | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_UnknownId_Then_Violation` |
| B2-DEL-R02 | Wrong preimage → fail | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_WrongPreimage_Then_Violation` |
| B2-DEL-R03 | `fulfillment_payload` > 32768 → error | DONE (3a54e11, e64da4e; a valid preimage is committed first; NL-325) | ONION M3b | `AT/Channels/Handlers/AttributionHandlerTests.Given_FulfillmentPayloadAbove32KiBWithAValidPreimage_…`, `…WithAWrongPreimage_…` [M3b] |
| B2-DEL-R04 | Malformed without BADONION → fail (NL-023) | DONE (engine N4; wired N6, a604dff/a02afa7) (NL-023) | N4-T2 | `…Given_MalformedWithoutBadOnion_Then_Violation` [M3] |
...
| B2-DEL-R07 | Double removal is a violation | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T2 | `…Given_DoubleFulfill_Then_Violation` |
...
| B2-CS-S01 | No CS without changes | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T3 | `DT/…/SendCommitTests.Given_NoChanges_Then_CannotSign` |
...
| B2-CS-R02 | `num_htlcs` mismatch → fail | DONE (engine N4; wired N6, a604dff/a02afa7) | N4-T3 | `DT/…/ReceiveCommitTests.Given_WrongNumHtlcs_Then_Violation` |
...
| B2-CS-R05 | Respond with RAA | DONE (a604dff) | N6-T1 | `…Given_ValidCommit_Then_RevokeAndAckQueued` |
| B2-CS-R06 | Persist before reply | DONE (a604dff; `CommitmentSignedMessageHandlerTests.Given_PersistFails_When_CommitmentSigned_Then_NoRevokeSentAndNoSecretReleased`) | N5-T2, N6-T1 | `…Given_PersistFails_Then_NoRevokeSent` |
...
| B2-RAA-R01 | Invalid secret → error + fail | DONE (`PerCommitmentSecretVerifier` + engine `MustFailChannel`; handler a604dff) | N3-T3, N6-T1 | `BT/…/PerCommitmentSecretVerifierTests`; `AT/…/RevokeAndAckMessageHandlerTests.Given_WrongSecret_Then_ErrorAndFail` |
...
| ID | Requirement | Status | Task | Test |
|---|---|---|---|---|
| B2-RE-W01 | `next_funding` = TLV 1 (txid ‖ flags) | DONE (NL-197) | N0-T6 | `ST/Messages/TxChannelReestablishMessageTests.Given_NextFundingType1_Then_Parsed` |
| B2-RE-W02 | TLV 5 `my_current_funding_locked` | DONE (ignored as unknown odd; not modeled) | N0-T6 | `…Given_Tlv5_Then_Ignored` |
| B2-RE-01 | Funder remembers after broadcast only | DONE (NL-048 wontfix: not remembered before broadcast; test 82c4c37 in `AT/Channels/Reestablish/FunderRememberRuleTests`) | N7-T6 | `AT/…/FundingSignedMessageHandlerTests.Given_FundingSigned_Then_PersistedBeforeBroadcast` |
| B2-RE-02 | Fundee remembers after `funding_signed` | DONE | — | `FundingCreatedMessageHandlerTests` |
| B2-RE-03 | Continue over a new transport | DONE (4ec83d3) | N7-T2 | `AT/Channels/Managers/ChannelManagerReconnectTests` |
| B2-RE-04 | Revert uncommitted peer updates; keep preimages | DONE (engine `RevertUncommitted` keeps `KnownPreimage`; run at connect and disconnect, 4ec83d3) | N4-T3, N7-T2 | `DT/…/ReestablishTests.Given_ProposedRemoteAdds_When_Disconnect_Then_Dropped_And_PreimageKept` |
| B2-RE-05 | Failed channel re-sends error | DONE (4ec83d3; error re-sent on every connection, no disconnect) | N6-T3, N7-T3 | `AT/…Given_FailedChannel_When_Reconnect_Then_ErrorResent` |
| B2-RE-06 | Send reestablish per channel | DONE (4ec83d3; also for V1FundingSigned/ReadyFor* since 22c29ae) | N7-T2 | `…Given_TwoOpenChannels_When_PeerInit_Then_TwoReestablishSent` |
| B2-RE-07 | Wait for peer's reestablish | DONE (4ec83d3 gating; `Publish` drops updates on a replaced connection, 22c29ae) | N7-T2 | `…Given_NotReestablished_When_LocalAdd_Then_Queued` |
| B2-RE-08 | `next_commitment_number` = L+1 | DONE (planner 4620895) | N7-T1 | `DT/Channels/Reestablish/ReestablishPlannerTests` |
| B2-RE-09 | `next_revocation_number` | DONE (planner 4620895) | N7-T1 | same |
| B2-RE-10 | Valid `my_current_per_commitment_point` | DONE (planner 4620895) | N7-T1 | same |
...
| B2-RE-18 | Resend last CS + updates, same number | DONE (4ec83d3; `SentCommitDiff` byte-identical) | N7-T3 | `…Given_LastCsLost_Then_UpdatesAndCsResent_SameNumber` |
| B2-RE-19 | Otherwise mismatch → fail | DONE (planner 4620895) | N7-T1 | `…Given_NumberAhead_Then_Error` |
...
| B2-RE-27 | Handle broadcast of any sent commitment | DONE (41f5fc2 `OnchainChannelWatcher` classifies current, next and revoked commitments; 3794c0d, 202341b, e5a556d resolve them; Docker O2, O4 (d), O5) | BOLT 5 | BOLT 5 plan [B5] |
...
| X-06 | Fail-the-channel service | DONE (Failed + `ErrorSent` a604dff; `ChannelFailureService` broadcast 36d2270, 06da54b, 983b2b4; broadcast row in the Failed save 41f5fc2, NL-271) | N6-T3, N9-T4 | `ChannelFailureServiceTests` |
...
| X-10 | Persist-before-send / crash consistency (I1–I12) | DONE for N4-N8 (invariant suite, N5-T3 crash injection, N6 wiring, N7 I11 crash at every save 1ad14ce; three-node SQLite restarts c4ad8e9) | N4-T5, N5-T3, N7-T3 | invariant suite, crash tests |
...
| N6-T2 `LocalOnlyHtlcSwitch` | M3-T1 + M3-T2 (create), done | Fail back via `IFailureOnionService.CreateErrorPacket`: final hop `incorrect_or_unknown_payment_details` (`temporary_node_failure` while no height is known), non-final `temporary_node_failure` (done, a02afa7, aac5f60) |
...
9. **`option_data_loss_protect` is advertised** (Optional, an ASSUMED bit) while reestablish is missing; peers assume RE-23 semantics. N7 is mandatory before any peer relies on it; N9-T4 checks `DataLossDetected`.
10. **Feature advertisement:** keep simple_close, anchors, quiesce, route_blinding, attribution_data, dual_fund, basic_mpp off until their milestones pass. They default to No, and `FeatureOptions.GetValidationErrors` refuses them unless `Features:AllowExperimentalFeatures=true` (NL-206); remove a feature from `FeatureOptions.ExperimentalFeatures` when its milestone lands.
...
15. **Test discovery:** done (N0-T1); `scripts/check-sln-configs.py` fails CI if a test project loses its runner.
